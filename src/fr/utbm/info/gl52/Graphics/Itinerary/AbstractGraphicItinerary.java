...
public abstract class AbstractGraphicItinerary extends AbstractGraphicElement {
	...
	public Point getPoint(int offset) {
		...
		if (p > 0 && p < it.getlRoute().size()) {
			Segment s = it.getlRoute().get(p);
			float xA = (float) (((Node<ESRISpatialObject>) s.getNodeA()).getData().getX()) - naturalOffset.x;
			float yA = (float) (((Node<ESRISpatialObject>) s.getNodeA()).getData().getY()) - naturalOffset.y;
			float xB = (float) (((Node<ESRISpatialObject>) s.getNodeB()).getData().getX()) - naturalOffset.x;
			float yB = (float) (((Node<ESRISpatialObject>) s.getNodeB()).getData().getY()) - naturalOffset.y;
